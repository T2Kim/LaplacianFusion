...
from lib.LaplCal import get_uniform_laplacian_1, LapEditor
...
parser.add_argument("--target_subj", default='hyomin_raise_hand_test')
...
# parser.add_argument("--target_subj", default='00114')
# parser.add_argument("--target_gender", default='female')
# parser.add_argument('--RGBD', default=True, help='Is Point Cloud?')
# parser.add_argument('--flathand', default=True)
...
# parser.add_argument("--target_subj", default='rp_alexandra_posed_006_ATU')
# parser.add_argument("--target_gender", default='female')
# parser.add_argument('--RGBD', default=True, help='Is Point Cloud?')
# parser.add_argument('--flathand', default=False)
# parser.add_argument('--epoch', default=100)

# parser.add_argument("--target_subj", default='hyomin_dance2')
# parser.add_argument("--target_gender", default='male')
# parser.add_argument('--RGBD', default=True, help='Is Point Cloud?')
# parser.add_argument('--flathand', default=False)
# parser.add_argument('--epoch', default=30)
...
if __name__ == '__main__':
    ...
    # ppcd = mmesh.sample_points_poisson_disk(100)
    ...
    L_dense = get_uniform_laplacian_1(anchor["shead_" + density_postfix + "_tri"])
    ...
    for i in tqdm(range(recon_frames), desc="Laplacian reconstruction"):
        ...
        # M_coarse = np.nan_to_num(M_coarse, nan=1)

        # mmesh = o3d.geometry.TriangleMesh()
        # mmesh.vertices = o3d.utility.Vector3dVector(coarse_verts[i])
        # mmesh.triangles = o3d.utility.Vector3iVector(f)
        # o3d.visualization.draw_geometries([mmesh])

        m_new = (shead_S @ np.squeeze(np.asarray(M_coarse.sum(1)))) / density

        delta_smpl = L_dense @ shead_S @ coarse_vertex_w_hand[i]
        # delta_smpl /= m_new[:, np.newaxis]
        # delta_smpl[anchor["shead2nohand_" + density_postfix]] = mapped_delta[i] * m_new[anchor["shead2nohand_" + density_postfix], np.newaxis]
        delta_smpl[anchor["shead2nohand_" + density_postfix]] = mapped_delta[i] * 0.000000001

        delta_new = delta_smpl

        tmp_coarse_verts = coarse_verts[i]
