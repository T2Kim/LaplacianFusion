...
# parser.add_argument("--target_subj", default='NeuroGIF_JacketPants1')
# parser.add_argument("--target_frame", default=0)
...
# parser.add_argument("--target_subj", default='corey_tilt')
# parser.add_argument("--target_frame", default=16)
# parser.add_argument('--epoch', default=330)
# parser.add_argument("--target_gender", default='male')

# parser.add_argument("--detail_subj", default='carla1')
# parser.add_argument("--detail_frame", default=0)
# parser.add_argument('--detail_epoch', default=1000)

# parser.add_argument("--detail_subj", default='NeuroGIF_JacketPants1')
# parser.add_argument("--detail_frame", default=0)
# parser.add_argument('--detail_epoch', default=200)

# parser.add_argument("--detail_subj", default='corey_tilt')
# parser.add_argument("--detail_frame", default=22)
# parser.add_argument('--detail_epoch', default=330)
...
# parser.add_argument("--target_subj", default='corey_tilt')
# parser.add_argument("--target_frame", default=28)
# parser.add_argument('--epoch', default=330)
# parser.add_argument("--target_gender", default='male')
# parser.add_argument("--detail_subj", default='eric_hips')
# parser.add_argument("--detail_frame", default=5)
# parser.add_argument('--detail_epoch', default=500)
...
# parser.add_argument('--epoch', default=330)
# parser.add_argument("--target_gender", default='male')
# parser.add_argument("--detail_subj", default='alexandra')
# parser.add_argument("--detail_frame", default=0)
# parser.add_argument('--detail_epoch', default=1000)
...
# parser.add_argument('--epoch', default=330)
# parser.add_argument("--target_gender", default='male')
# parser.add_argument("--detail_subj", default='cindy')
# parser.add_argument("--detail_frame", default=0)
# parser.add_argument('--detail_epoch', default=1000)

# parser.add_argument("--target_subj", default='eric_hips')
...
# parser.add_argument("--target_gender", default='male')
# parser.add_argument("--detail_subj", default='corey_tilt')
# parser.add_argument("--detail_frame", default=28)
# parser.add_argument('--detail_epoch', default=330)
...
# parser.add_argument("--target_subj", default='alexandra')
# parser.add_argument("--target_frame", default=0)
# parser.add_argument('--epoch', default=1000)
# parser.add_argument("--target_gender", default='female')
# parser.add_argument("--detail_subj", default='carla1')
...
if __name__ == '__main__':
    ...
    detail_dir = os.path.join(cfg.DataPath["Main"], "subjects", args.detail_subj)
    ...
    recon_dir = os.path.join(target_dir, "application", "trans_" + args.detail_subj)
    ...
    S = sp.sparse.load_npz(os.path.join(cfg.DataPath["Main"], "protocol_info_new/nohand_" + density_postfix + ".npz" ))
    n_dense_v = S.shape[0]
    ...
    coarse_verts = np.zeros((recon_frames, len(anchor["shead_coord"]), 3))
    ...
    #region detail inference
    model_lap = MLP_Detail(3, 84).to(device)
    model_lap.load_state_dict(torch.load(os.path.join(detail_dir, "net/lap_model" + "_e" + str(args.detail_epoch) + ".pts")))
    model_lap.eval()
    params = load_params(os.path.join(detail_dir, "train/smplx_fit.npz"), device, dtype)
    (input_body_pose_detail, _, _,
        _, _, 
        _, _) = params
    detail_recon_frames = len(input_body_pose_detail)

    pose_map = get_posemap_custom().float().to(device)
    #region Inference delta
    cfg.rootLogger.info("Inference delta")
    pose_code = matrix_to_quaternion(batch_rodrigues(input_body_pose_detail.reshape(-1, 3))).reshape(detail_recon_frames, 21, 4)
    all_infer_count = n_dense_v * detail_recon_frames
    mapped_delta_detail = np.zeros((detail_recon_frames, S.shape[0], 3))
    n_infer_once = cfg.lap_infer_frame_max * (n_dense_v + 1)
    ...
    for i in tqdm(range((all_infer_count // n_infer_once) + 1), desc="inference delta"):
        ...
        with torch.no_grad():
            pose_map_sub_verts = torch.where(torch.einsum('ij, jk -> ik', body_skin_weight[tmp_index_vert], pose_map) > 0, 1., 0.) # n_sub_verts x 21
            mapped_delta_detail[tmp_index_frame, tmp_index_vert, :] \
                    = model_lap(manifold_coord_gpu[tmp_index_vert], (pose_map_sub_verts.unsqueeze(-1) * pose_code[tmp_index_frame]).reshape(-1, 84)).cpu().numpy()
    #endregion

    mapped_delta_detail_trans = np.zeros((recon_frames, S.shape[0], 3))
    mapped_delta_detail_trans = np.repeat(mapped_delta_detail[args.detail_frame][np.newaxis, :, :], recon_frames, axis=0)
    ...
    leye_pose=torch.zeros([1, 3], dtype=dtype, device=device)
    reye_pose=torch.zeros([1, 3], dtype=dtype, device=device)
    mapped_delta_detail_trans = body_model.LBS_deform(custom_lbs_weight, torch.tensor(mapped_delta_detail_trans, dtype=dtype, device=device),
                            only_rotation = True, inverse = False,
                            body_pose=input_body_pose,
                            betas=input_betas_1,
                            global_orient=input_global_orient,
    ...
    picked_points = np.concatenate((anchor["pre_anchor_idx_2000"], anchor["hand_foot_idx"]))
    picked_points = np.concatenate((anchor["pre_anchor_idx"], anchor["hand_foot_idx"]))
    # picked_points = np.arange(len(anchor["shead_coord"]))
    ...
    smplx_S = sp.sparse.load_npz(os.path.join(cfg.DataPath["Main"], "protocol_info_new/smplx_" + density_postfix + ".npz" ))
    smplx_verts = np.zeros((recon_frames, smplx_S.shape[0], 3))

    for i in range(recon_frames):
        smplx_verts[i] = smplx_S @ smplx_verts_[i]

    shead_S = sp.sparse.load_npz(os.path.join(cfg.DataPath["Main"], "protocol_info_new/shead_" + density_postfix + ".npz" ))

    coarse_vertex_w_hand = smplx_verts[:, anchor["smplx2shead"], :]
    coarse_vertex_w_hand = coarse_verts
    ...
    L_dense = igl.cotmatrix(v_sub_dense, anchor["shead_" + density_postfix + "_tri"])
    ...
    for i in tqdm(range(step), desc="Laplacian reconstruction"):
        ...
        m_new = (shead_S @ np.squeeze(np.asarray(M_coarse.sum(1)))) / density

        delta_smpl = L_dense @ shead_S @ coarse_vertex_w_hand[demo_frame]
        # delta_smpl[anchor["shead2nohand_" + density_postfix]] = mapped_delta[demo_frame] * m_new[anchor["shead2nohand_" + density_postfix], np.newaxis]

        delta_smpl[anchor["shead2nohand_" + density_postfix]] = ( (1 - (i / (step - 1))) * mapped_delta[demo_frame] + ((i) / (step - 1)) * mapped_delta_detail_trans[demo_frame]) * m_new[anchor["shead2nohand_" + density_postfix], np.newaxis]

        delta_new = delta_smpl
        # delta_new[anchor["shead2nohand_" + density_postfix]] = tmp_delta_new

        tmp_coarse_verts = coarse_verts[demo_frame]
    ...
    if cfg.VISUALIZE:
        ...
        for frame_idx in range(step):
            pcd_list_o3d.append(text_3d("Transfer X {:.3f}".format(((frame_idx) / (step - 1))), init_pos + np.array([-0.5,1,0])))

        o3d_nb_vis({"Mesh0" : {"vertices":recon_vertex_t, "triangles": anchor["shead_" + density_postfix + "_tri"]},
                    "O3D_PCD0" : {"pcd":pcd_list_o3d}})

    # Save
    for t in tqdm(range(step), desc="Save"):
        # basename = os.path.basename(coarse_files[t])
        basename = str(t).zfill(6)
        filename_recon = os.path.join(recon_dir, os.path.splitext(basename)[0] + ".ply")
